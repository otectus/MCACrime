...
- [ ] **Alive capture pays a multiplier.** Restrain a Wanted outlaw with rope/cuffs. Have a guard
      ...
      them (citizen's arrest by capture + guard proximity). You receive emeralds per config
      `bounty.aliveCaptureMultiplier` (default 1.25× the kill amount). Message reads `Bounty
      collected (alive): X emeralds for Y.`
