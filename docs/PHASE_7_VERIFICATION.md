...
**Every check below needs a real client and dedicated server with MCA Reborn 7.6.x or 7.7.x, the
`mcacrime` jar, and preferably two clients (one as the player, one observing).**
...
- [ ] **Thieves spawn and scout.** Assign a thief with `/crime job assign thief`. Visit the
      village a few times over several minutes. Eventually a villager changes profession to Thief
      (if `presentThiefAsMcaProfession = true`) and begins scouting the area, looking at players
      and moving around.
...
- [ ] **Thief steals currency first.** Let a thief mug you successfully (no weapon, no guard
      ...
      some emeralds (or equivalent currency per `integrations.currencyId`). HUD shows outcome
      `stolen` with currency amount.
...
- [ ] **Protected slots are immune.** Carry a crafting table in hotbar, a diamond helmet in armor,
      ...
      `criminalJobs.thief.protectHotbar`, `protectArmor`, `protectOffhand` (all true by default).
...
- [ ] **Fence job assignment.** `/crime job assign fence`. The nearest villager becomes a Fence
      (changes profession if `presentFenceAsMcaProfession = true`).
...
- [ ] **Pricing reflects karma.** Raise your karma to Lawful (above `bands.karmaBlueThreshold`, default
      +100) and trade with the fence. Buy prices are lower than a Neutral baseline. Lower karma to
      Outlaw (below `bands.karmaRedThreshold`, default −100) and return to trade. Buy prices are higher
      (surcharge). Check config `criminalJobs.fence.maxKarmaDiscount` (default 0.25, 25%
      discount for Lawful).
- [ ] **Pricing reflects Heat.** Get caught committing crimes so Heat rises. Trade with the fence
      at high Heat. Prices increase (surcharge per `criminalJobs.fence.maxHeatMarkup`, default 0.35). Heat decays
      back to zero and prices drop again.
- [ ] **Pricing reflects Wanted.** Get Heat above `bands.wantedHeatThreshold` (default 50) so you are
      Wanted. Trade with the fence. Prices increase by the Wanted surcharge (`criminalJobs.fence.wantedMarkup`,
      default 0.20). Surrender to a guard to drop Heat, lose Wanted status, and prices return.
...
- [ ] **(With Locks Reforged installed)** **Locks are sold by fences.** Have `locks_reforged.jar`
      in `run/mods`. Trade with a fence and see locks, picks, and keys in the offers (if
      `integrations.locksReforgedFenceTrades = true`, default). Prices tier by lock type (wood <
      copper < iron < steel < gold < diamond < netherite).
- [ ] **(Without Locks Reforged)** **No errors when absent.** Remove `locks_reforged.jar` from
      `run/mods`. Trade with a fence. No `ClassNotFoundException`, no log errors. Fence offers only
      tag-driven contraband (TNT, fire charge, restraint items, etc. per
      `data/mcacrime/tags/items/illicit_goods.json`). `/crime reload` prints no Locks warnings.
...
- [ ] **Bounty for killing a Wanted outlaw.** Get a villager or player to Wanted state (Heat ≥ 50,
      Wanted band). Check `bounty.payForKills = true` (default). Kill the outlaw with a weapon
      (not fall damage, suffocation, or lava). You receive the active currency as payment (emeralds
      by default; see `integrations.currencyId`) and message `Bounty collected: X emeralds for Y.`
      Your karma also increases slightly per config `bounty.karmaReward`.
...
- [ ] **Alive capture pays a multiplier.** Restrain a Wanted outlaw with rope/cuffs. Have a guard
      nearby (within `bounty.deliveryRadius`, default 4.0 blocks) and let the guard arrest
      them (citizen's arrest by capture + guard proximity). You receive the active currency per
      config `bounty.aliveCaptureMultiplier` (default 1.25× the kill amount). Message reads
      `Bounty collected (alive): X emeralds for Y.`
...
## H. Dedicated server — MCA 7.6 and 7.7, with/without optional companions
...
- [ ] `/crime job assign thief [target]` works; thief is assigned in `/crime debug thieves` output.
- [ ] `/crime validate` runs without crashing.

### H2: MCA 7.6.20 + `mcaquests-1.5.3.jar`
- [ ] No `NoClassDefFoundError`, no `ClassNotFoundException` in logs.
...
### H3: MCA 7.6.20 + `locks_reforged-1.7.4.jar`
- [ ] No `NoClassDefFoundError`, no `ClassNotFoundException` in logs.
- [ ] `/crime debug integrations` reports `locksreforged=on` (or equivalent bridge status).
- [ ] Fences can be assigned and traded with unarmed.
- [ ] Check logs for Locks tier pricing contributions (search for `LocksReforgedCompat`).

### H4: MCA 7.6.20 + both `mcaquests-1.5.3.jar` and `locks_reforged-1.7.4.jar`
- [ ] No `NoClassDefFoundError`, no `ClassNotFoundException` in logs.
- [ ] Both integrations report as active in `/crime debug integrations`.
- [ ] Thieves can be assigned and mugged; bounty system works; fences trade with locks.

### H5: Repeat all four scenarios with MCA 7.7.1
- [ ] Same checks as H1–H4, against the 7.7.1-alpha.2 probe jar instead.
