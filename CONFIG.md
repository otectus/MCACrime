...
mugged: `criminalJobs.villageThiefChance` `0.025` → `0.01`; `criminalJobs.assignmentScanIntervalTicks`
`1200` → `2400`; `criminalJobs.thief.mugCooldownTicks` `12000` → `24000`. All three are unchanged
keys with new default values — nothing was renamed or removed. See
...
throttles remain a declared seam that changes nothing, and turning it on produces a validator
warning saying so. That is separate from `[npccrime.accomplices]` below, which **is** shipped: it is
player-initiated crime with a villager accomplice, not NPC-initiated crime, so it is not gated on
`enableNpcCrime` at all.

Two more keys in this section are wired, independent of `enableNpcCrime`: they tune the mugging
sessions a Thief villager already starts (see `[criminalJobs]`).

| Option | Default | Range | What it does |
|---|---|---|---|
| `npcMugHudUpdateIntervalTicks` | `3` | `1 … 20` | Ticks between progress packets for a thief's mugging bar sent to the victim's client. The client interpolates between updates, so this controls packet volume rather than smoothness. |
| `npcMugWeaponCheckIntervalTicks` | `1` | `1 … 10` | Ticks between checks of whether the mugging victim has drawn a weapon, which stops the mug. `1` checks every tick. |
...
**The three actions.** Each is reached from the Crime menu's `Conspire` group and requires the
target to be family within `accompliceScope`, at or above `accompliceHeartsRequired`, and not
already an active accomplice on cooldown:
...
arrest of that same relative. Paying uses the same currency sink as a player's own self-bail. If the
player's balance is below the quoted cost, or the relative has already been released between the
quote and the payment, nothing is charged. Without payment, custody simply runs to the end of the
sentence like any other arrest.
...
| `searchLosTicksRequired` | `40` | `0 … 1200` | Ticks of unbroken line of sight a guard must accumulate, in `searchIntervalTicks` steps per pass, before it searches. `0` means a glance is enough; at the defaults (`40`/`40`) a single pass with line of sight already satisfies it. |
...
(`data/mcacrime/tags/items/illicit_goods.json`), which lists `minecraft:tnt`,
`minecraft:fire_charge`, `minecraft:gunpowder`, `minecraft:wither_skeleton_skull`,
`minecraft:spyglass`, `minecraft:tripwire_hook`, `minecraft:ender_pearl`, `minecraft:golden_apple`,
`minecraft:name_tag`, and this mod's own restraint items — a starting point for a pack, not a
default: `illegalItems` itself ships empty and this tag is not added to it automatically.

**Tags are validated late, on purpose.** Item tags do not exist while the config loads, so a
`#tag` entry cannot be checked at config-load time at all. It is provisionally accepted, then
checked once tags actually bind (on `TagsUpdatedEvent`); an entry naming a tag nobody registered is
dropped from the active list at that point, with one log line naming it, and the rest of the list
keeps working. Nothing about a malformed or unknown entry is ever fatal.
...
sight, counted in `searchIntervalTicks` steps per pass (at the defaults, one pass with line of
sight is enough); `searchRequiresSuspicion` is off or the player is already wanted/suspicious; and a
...
order-independent fingerprint over the listed items' ids and counts — is not charged again within
`rechargeTicks` of the last charge; a haul that changes (a different item, a different count) is
charged immediately regardless of the recharge window.
...
would mean rewriting its block-entity NBT. There is no recovery ledger for a confiscated item — it
...
witness set itself, so the crime is un-witnessed for Heat, community standing and family heart loss
alike — they still remember it and can talk about it. On by default.
...
**The decision.** Deterministic, no RNG: two identical crimes minutes apart produce the same
answer. The score is `loyaltyHeartsWeight × hearts + tierBonus + personalityModifier`, and a
relative stays quiet when that score is at or above `loyaltyThreshold` **and** none of six hard
exclusions apply — an exclusion always wins, however high the score:
