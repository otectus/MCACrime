...
This is the NeoForge 1.21.1 port of MCA: Crime. It is a separate project from the Forge 1.20.1 baseline. Registration inventory and package layout live in `MODMAP.md` (generated by `.mcmod-tools/modmap.py`). Read that instead of crawling `src/`.
...
compat integration  the only packages allowed to reach a foreign mod
...
- Registration is imperative on the MOD bus in the `McaCrime` constructor. Gameplay handlers are `@EventBusSubscriber` (inherits FORGE bus) or on the mod bus. NeoForge uses a different annotation system; check the imports (`net.neoforged.*`).
...
- Exactly one mixin: `mixin/client/PlayerModelRestraintMixin`, one `@Inject` at TAIL, guarded by `MixinConfigTest`. **MixinExtras is neither declared nor used** - no `@WrapOperation`.
...
- World file schema version: 6 (allows schema migrations, not rewrites)
- Config keys: common and client sections, exact keys stable across patches
- Packet protocol version: 7 (1.20.1 clients cannot join)
