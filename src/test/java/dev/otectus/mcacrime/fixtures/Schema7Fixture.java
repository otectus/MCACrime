 ...
 * in as a {@code .dat} for two reasons. The first is that there is no {@code src/test/resources} in
 * this project and adding one for a binary blob would mean nobody could ever read the input to a
 * failing assertion. The second is more important: the tags below are written <b>by hand</b>, not by
 * calling the mod's own {@code save} methods. A fixture that serialised live objects would silently
 * acquire every field 0.6.0 adds the moment it was added — {@code sentenceId} on a case,
 * {@code surrenderCredited} on a jail state, {@code paidAmount} on a bounty claim — and would then be
 * asserting that the new code can read its own output, which is the one thing nobody doubts. What
 * needs proving is that it can read what the <em>old</em> jar produced, so the old shapes are frozen
 * here and only change when somebody decides they should.
 *
 ...
 * and there is no Minecraft bootstrap on this classpath; an empty list is a cell whose record is real
 * and whose restoration journal has nothing in it, which is exactly the case worth exercising anyway.
 */
