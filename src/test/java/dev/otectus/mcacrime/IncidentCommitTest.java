...
class IncidentCommitTest {
    private static CrimeRecord record(UUID id) {
        return new CrimeRecord(id, UUID.randomUUID(), UUID.randomUUID(), new ResourceLocation("mcacrime", "harm_villager"),
    ...
    @Test void replayAfterSaveReloadDoesNotRepeatTheAct() {
        ...
        var loaded = CrimeWorldData.load(world.save(new CompoundTag()));
    ...
    @Test void futureStoreRejectsBeforeCallbacks() {
        ...
        var world = CrimeWorldData.load(tag);
    ...
    @Test void provenanceSurvivesSaveWithoutSchemaChange() {
        ...
        CompoundTag tag = world.save(new CompoundTag());
        assertEquals(CrimeDataMigrations.CURRENT_SCHEMA, tag.getInt(CrimeDataMigrations.TAG_SCHEMA));
        assertEquals(record.context(), CrimeWorldData.load(tag).recordById(record.id()).orElseThrow().context());
    }
}
