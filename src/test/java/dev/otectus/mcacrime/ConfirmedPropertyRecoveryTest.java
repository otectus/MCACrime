...
class ConfirmedPropertyRecoveryTest {
    ...
    static CompoundTag stack(int count) {
        var tag = new CompoundTag(); tag.putString("id", "minecraft:diamond"); tag.putInt("count", count);
        var custom = new CompoundTag(); custom.putString("name", "Recovered heirloom"); var components = new CompoundTag(); components.put("minecraft:custom_data", custom); tag.put("components", components); return tag;
    }
    ...
    @Test void futureStoreRefusesRecoveryAndDelivery() {
        ...
        var frozen = CrimeWorldData.load(tag, net.minecraft.core.RegistryAccess.EMPTY); assertEquals(0, StolenGoodsLedger.escrowAll(frozen, thief, 10));
    ...
    @Test void itemProvenanceAndProviderAreDefensivelyCopied() {
        ...
        original.putInt("count", (int)40); record.stackTag().putInt("count", (int)20);
        assertEquals(3, record.stackTag().getInt("count")); assertEquals(record, StolenGoodsRecord.load(net.minecraft.core.RegistryAccess.EMPTY, record.save()));
        data.putStolenGoods(record); StolenGoodsLedger.escrowAll(data, thief, 10);
        var owed = data.propertyEscrowFor(owner).get(0); owed.stackTag().putInt("count", (int)30);
    ...
    @Test void oldCurrencyRecordsRemainExplicitlyUnidentified() {
        ...
        assertEquals("", StolenGoodsRecord.load(net.minecraft.core.RegistryAccess.EMPTY, legacy.save()).providerId());
    }
    ...
    @Test void ambiguousReceiptSurvivesReloadAndRetention() {
        ...
        var loaded = CrimeWorldData.load(data.save(new CompoundTag(), net.minecraft.core.RegistryAccess.EMPTY), net.minecraft.core.RegistryAccess.EMPTY); loaded.pruneTransactions(Long.MAX_VALUE / 2);
    ...
    @Test void deliveryIdIsStableAcrossNbtRoundTrip() {
        var lot = new PropertyLot(UUID.randomUUID(), owner, stack(3), 0, "", transaction, PropertyLot.DeliveryState.PENDING, 0);
        assertEquals(PropertyEscrow.deliveryId(lot), PropertyEscrow.deliveryId(PropertyLot.load(net.minecraft.core.RegistryAccess.EMPTY, lot.save())));
    }
