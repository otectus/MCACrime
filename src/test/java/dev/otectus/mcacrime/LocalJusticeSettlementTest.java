...
class LocalJusticeSettlementTest {
    ...
    private static final CrimeCommunityKey LOCAL = new CrimeCommunityKey(ResourceLocation.fromNamespaceAndPath("minecraft", "overworld"), 1);
    private static final CrimeCommunityKey REMOTE = new CrimeCommunityKey(ResourceLocation.fromNamespaceAndPath("minecraft", "overworld"), 2);
    ...
    @Test void jurisdictionIncludesDimensionAndNullIsNotGlobal() {
        var crime = record(1, 5, 20);
        var data = ledger(crime);
        report(data, crime, new CrimeCommunityKey(ResourceLocation.fromNamespaceAndPath("minecraft", "the_nether"), 1), 1F);
    ...
    @Test void futureDataCannotAuthorizeOrDebit() {
        ...
        var tag = original.save(new CompoundTag(), net.minecraft.core.RegistryAccess.EMPTY);
        tag.putInt("schema", Integer.MAX_VALUE);
        var future = CrimeWorldData.load(tag, net.minecraft.core.RegistryAccess.EMPTY);
