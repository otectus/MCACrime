...
public final class ClientActionData {
    ...
    /** The formatted outcome line, empty when the server sent a bare key. */
    private static volatile Component outcomeLine = Component.empty();
    ...
    /**
     * The outcome line to draw. Falls back to the bare key when the server sent no formatted text,
     * which is every outcome that takes no arguments and every packet from an older server.
     */
    public static Component outcomeText() {
        Component line = outcomeLine;
        return line.getContents() == ComponentContents.EMPTY
                ? Component.translatable(outcomeKey) : line;
    }
