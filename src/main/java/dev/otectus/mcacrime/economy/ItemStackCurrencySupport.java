 ...
 *
 * <p>Counting, spending and merging all require {@code stack.is(item)} with no data
 * components on it. A renamed emerald, an enchanted coin, a stack carrying an economy mod's own data,
 * or anything a player has
 * deliberately made special is <b>not</b> currency — a fine must never quietly consume somebody's
 * named keepsake because it happens to share an item id, and merging into a patched stack would erase
...
final class ItemStackCurrencySupport {
    ...
    /** How many unmodified {@code item} the player is carrying in their main inventory. */
    ...
    /** Removes up to {@code requested} unmodified {@code item}; returns how many were actually removed. */
    ...
    static List<ItemStack> toStacks(Item item, long amount) {
        ...
        for (int count : ItemCurrencyMath.splitCounts(amount, Math.max(1, item.getDefaultMaxStackSize()))) {
    ...
    static long stacksNeeded(Item item, long amount) {
        return item == null ? 0L : ItemCurrencyMath.stacksNeeded(amount, Math.max(1, item.getDefaultMaxStackSize()));
    }

    /** See the class note: patched stacks are somebody's property, not the server's small change. */
    private static boolean isMoney(ItemStack stack, Item item) {
        return stack != null && stack.is(item) && stack.getComponentsPatch().isEmpty();
    }
}
