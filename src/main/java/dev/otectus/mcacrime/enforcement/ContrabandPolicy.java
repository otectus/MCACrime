...
public final class ContrabandPolicy {
    ...
    private static boolean isKnownTag(ResourceLocation tag) {
        try {
            // 1.21.1 has no IForgeRegistry tag view: a tag exists exactly when the item registry can
            // hand back a holder set for it, which is the same question TagsUpdatedEvent just answered.
            return BuiltInRegistries.ITEM.getTag(TagKey.create(Registries.ITEM, tag)).isPresent();
        } catch (Throwable t) {
