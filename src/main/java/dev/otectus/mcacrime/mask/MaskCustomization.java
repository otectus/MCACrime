 ...
 *       thereby volunteered that helmet's NBT for rewriting.</li>
...
public final class MaskCustomization {
    ...
    public static Result restyleResult(@Nullable ItemStack source, @Nullable ItemStack target) {
        ...
        result.setCount(1);
        Optional<CompoundTag> merged = MaskNbtTransfer.merge(source.getTag(), result.getTag());
        if (merged.isEmpty()) {
            return Result.refused(MaskRestyleRejection.UNSUPPORTED_DATA);
        }
        CompoundTag tag = merged.get();
        result.setTag(tag.isEmpty() ? null : tag);
        // Set through the stack, not through the copied tag: this is the assertion that the two masks
        // really do share a maximum durability, and it is what keeps a restyle from being a repair.
     ...
     * source's root tag, and whether the operation would change the colour. No registry, no {@code
     * ItemStack}, no {@code Level} -- so "cross-family is refused, a no-op is refused, a capability is
     * refused, a foreign flag is carried" is a plain unit test rather than an in-world experiment.
     *
     ...
     * @param sourceTag    the source stack's root tag, or {@code null}
    ...
    public static MaskRestyleRejection decide(@Nullable MaskVariant from, @Nullable MaskVariant to,
                                              @Nullable CompoundTag sourceTag, boolean colorChanges) {
        ...
        if (!MaskNbtTransfer.unsupportedKeys(sourceTag).isEmpty()) {
    ...
    public static MaskRestyleRejection wouldChange(ItemStack source, ItemStack target, @Nullable DyeItem dye) {
        ...
        return decide(from, to, source.getTag(), colorChanges);
    }

    /**
     * Applies one dye's colour to a mask being made (section 7.2's optional dye slot).
     *
     * <p>Vanilla's own leather algorithm, not a private one: a dyed mask then reads as dyed to every
     * piece of machinery that already understands {@code display.color}, and one dye produces exactly
     * the colour the same dye produces on a leather cap. Appearance only -- no other field is touched.
     *
    ...
    public static ItemStack tint(ItemStack stack, DyeItem dye) {
        if (stack == null || stack.isEmpty() || dye == null
                || !(stack.getItem() instanceof DyeableLeatherItem)) {
            return stack;
        }
        ItemStack dyed = DyeableLeatherItem.dyeArmor(stack, List.of(dye));
        return dyed == null || dyed.isEmpty() ? stack : dyed;
    }

    /** The dye colour on a stack, or white when it carries none (matching {@code MaskItem#getColor}). */
