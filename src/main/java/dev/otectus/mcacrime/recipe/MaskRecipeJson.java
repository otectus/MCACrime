 ...
 *
 * <p>1.21 note: the result block is {@code {"id": ..., "count": 1}}. {@code result.item} was the
 * 1.20.1 spelling and is a load-time parse failure here, so it is refused by name rather than ignored.
 * The live parse in 1.21.1 runs through a {@code Codec}, which never sees a {@code JsonObject}, so
 * {@link #problems(String, String, int, int, int, int)} is the overload the serializer calls on the
 * values it decoded; this JSON overload stays the contract the resource tests hold shipped files to.
 */
public final class MaskRecipeJson {
    ...
    public static List<String> problems(JsonObject json) {
        ...
        if (result == null) {
            ...
            String item = string(result, "id").orElse(null);
            if (item == null) {
                problems.add("result.id: missing or not a string");
            } else if (ResourceLocation.tryParse(item) == null) {
                problems.add("result.id: '" + item + "' is not a resource location");
            }
            if (result.has("count")) {
    ...
    public static Optional<Fields> parse(JsonObject json) {
        ...
        return Optional.of(new Fields(
                ...
                ResourceLocation.tryParse(string(result, "id").orElseThrow()),
                1,
                json.has("sort_order") ? json.get("sort_order").getAsInt() : 0));
    }

    /**
     * The same bounds as {@link #problems(JsonObject)}, checked against values something else already
     * decoded — the codec path, and the network path a hostile server could otherwise widen.
     *
     * @param operation    the operation spelling, as written
     * @param group        the recipe group, never null
     * @param materialCount how many of the material one craft consumes
     * @param bindingCount  how many of the binding one craft consumes
     * @param resultCount   how many items the result block asks for
     * @param sortOrder     the catalogue sort key
     */
    public static List<String> problems(String operation, String group, int materialCount,
                                        int bindingCount, int resultCount, int sortOrder) {
        List<String> problems = new ArrayList<>();
        if (operation == null) {
            problems.add("operation: missing or not a string");
        } else if (MaskOperation.parse(operation).isEmpty()) {
        ...
        checkCount("material", materialCount, problems);
        checkCount("binding", bindingCount, problems);
        if (resultCount != 1) {
            problems.add("result.count: must be 1 for a mask operation, was " + resultCount);
        }
        if (Math.abs(sortOrder) > MAX_SORT_ORDER) {
            problems.add("sort_order: outside +/-" + MAX_SORT_ORDER);
        }
        return problems;
    }

    private static void checkCount(String slot, int count, List<String> problems) {
        if (count < 1) {
