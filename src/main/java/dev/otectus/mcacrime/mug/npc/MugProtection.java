...
public final class MugProtection {
    ...
    public static void grant(@Nullable ServerPlayer player, int ticks) {
        ...
        CrimeCapabilities.get(player).ifPresent(data -> data.setMugProtectionUntilTick(
    ...
    public static Window window(@Nullable ServerPlayer victim, @Nullable UUID thief, long now) {
        PlayerCrimeData data = victim == null ? null : CrimeCapabilities.get(victim).orElse(null);
        if (data == null) {
            return Window.NONE;
        }
