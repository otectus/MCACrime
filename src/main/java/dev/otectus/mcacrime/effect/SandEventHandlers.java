...
public final class SandEventHandlers {
    ...
    private static void endedEarly(LivingEntity entity, net.minecraft.world.effect.MobEffect effect) {
        if (effect == null || !CrimeEffects.SAND_BLINDED.isPresent()
                || effect != CrimeEffects.SAND_BLINDED.get() || !isServer(entity)) {
            return;
        }
        SandRecoveryLedger.cured(entity, SandRecoveryLedger.clock(entity), recovery());
    }

    private static boolean isSand(net.minecraft.world.effect.MobEffectInstance instance) {
        return instance != null && CrimeEffects.SAND_BLINDED.isPresent()
                && instance.getEffect() == CrimeEffects.SAND_BLINDED.get();
    }
