...
import dev.otectus.mcacrime.recipe.MaskStationInput;
 ...
 * small payload), and computes the same grid from the recipes vanilla already synced to it. It never
 * supplies the item, the count, the components or the entitlement — a selection payload names an id and
 * the session it was made in, and the server re-decides everything else (invariant 6).
 *
 ...
 *
 * <p>1.21.1 differences from the 1.20.1 original, all mechanical. A recipe no longer carries its own
 * id, so the selection is resolved against a {@link RecipeHolder} and its {@link RecipeHolder#id()};
 * and matching and assembly take a {@link MaskStationInput} snapshot of the working inventory rather
 * than the live {@code Container}. Every rule about who decides what is unchanged.
 */
public class MaskStationMenu extends AbstractContainerMenu {
    ...
    /** The same three stacks as a recipe input, which is what 1.21 hands to {@code matches}. */
    public MaskStationInput recipeInput() {
        return MaskStationInput.of(inputs);
    }
    ...
    private void refresh() {
        ...
        MaskStationInput input = recipeInput();
        List<MaskStationCatalog.Entry> entries = new ArrayList<>();
        List<RecipeHolder<MaskMakingRecipe>> matching = new ArrayList<>();
        if (craftingEnabled()) {
            for (RecipeHolder<MaskMakingRecipe> holder : manager.getAllRecipesFor(CrimeRecipes.MASK_MAKING.get())) {
                MaskMakingRecipe recipe = holder.value();
                if (!operationEnabled(recipe) || !recipe.matches(input, level)) {
                    continue;
                }
                matching.add(holder);
                entries.add(recipe.catalogEntry(holder.id()));
            }
        }
        ...
        selectedRecipe = selected == null ? null
                : matching.stream().filter(holder -> holder.id().equals(selected)).findFirst().orElse(null);
        if (selectedRecipe == null) {
            selected = null;
        }

        ItemStack assembled = selectedRecipe == null ? ItemStack.EMPTY
                : selectedRecipe.value().assemble(input, level.registryAccess());
    ...
    private MaskCraftRejection emptyPreviewReason() {
        if (selectedRecipe != null && selectedRecipe.value().operation() == MaskOperation.RESTYLE
                && !selectedRecipe.value().restyleRejection(recipeInput()).allowed()) {
    ...
    private MaskCraftPlan preparePlan(int batch, int destinations) {
        ...
        RecipeHolder<MaskMakingRecipe> holder = selectedRecipe;
        if (holder == null) {
        ...
        MaskStationInput input = recipeInput();
        return MaskCraftPlan.prepare(recipe.materialCount(), recipe.bindingCount(), recipe.dyeCount(input),
    ...
    private int countRemainders(MaskMakingRecipe recipe) {
        int count = 0;
        for (ItemStack stack : recipe.getRemainingItems(recipeInput())) {
            if (!stack.isEmpty()) {
                count++;
            }
        }
    ...
    private void consumeOne(MaskMakingRecipe recipe) {
        int dye = recipe.dyeCount(recipeInput());
