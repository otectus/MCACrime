 ...
 * {@code PathNavigation#getMob} does not exist in 1.21.1, and a {@code Brain} has never known whose
 * it is.
 *
 ...
 * {@code VillagerServerTickMixin} injects at the TAIL of {@code aiStep} -- by that Mojang name on
 * its NeoForge branch -- and calls {@code VillagerServerTickDispatcher.tick}, which is what runs
 * {@code GuardRestEnforcerTicker} and the rest. NeoForge fires {@code EntityTickEvent.Pre} from
 * {@code ServerLevel.tickNonPassenger}, immediately before {@code Entity.tick()}: the same tick of
 * the same entity, on the same thread, and strictly earlier. So a context recorded from that event is
 * exactly the villager whose Townstead tickers run next.
 *
...
public final class TownsteadTickContext {
     ...
     * {@code EntityTickEvent.Pre} handler, which already runs for every entity on the server.
     */
