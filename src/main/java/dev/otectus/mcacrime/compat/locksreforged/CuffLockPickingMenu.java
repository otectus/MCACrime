...
import melonslise.locks.common.network.LocksSync;
import melonslise.locks.common.init.LocksSoundEvents;
import melonslise.locks.common.init.LocksTagHelper;
import melonslise.locks.common.network.toclient.TryPinResultPayload;
...
public final class CuffLockPickingMenu extends LockPickingContainer {
    ...
    public static boolean open(ServerPlayer player, CustodyRecord custody) throws ReflectiveOperationException {
        ...
        boolean sequenced;
        try {
            result = TryPinResultPayload.class.getConstructor(int.class, int.class, int.class, int.class,
                    boolean.class, boolean.class, boolean.class);
            sequenced = true;
        } catch (NoSuchMethodException legacy) {
            result = TryPinResultPayload.class.getConstructor(boolean.class, boolean.class);
            sequenced = false;
        }
        ...
        var item = BuiltInRegistries.ITEM.getOptional(ResourceLocation.fromNamespaceAndPath("locks", "iron_lock")).orElse(null);
        ...
        player.openMenu( new SimpleMenuProvider((id, inventory, actor) ->
    ...
    /** Legacy unsequenced packet entry point. Never call the block-lock implementation. */
    public void tryPin(int pin) {
        if (!sequenced) attempt(pin, progress.lastSequence() + 1);
    }

    /** Sequenced Locks protocol; both entry points remain compatible across native API revisions. */
    ...
    private void send(Object result) {
        LocksSync.sendTo((ServerPlayer) player, (TryPinResultPayload) result);
    }
