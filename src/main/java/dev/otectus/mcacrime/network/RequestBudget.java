...
public final class RequestBudget {
    ...
    /**
     * Default clock: real time in ticks. The registrar runs these handlers on the main thread, but the
     * bucket is keyed by player rather than by level and a refusal must be answerable before any level
     * lookup happens, so wall time is the clock that is always available.
     */
