...
public final class WarrantService {
    ...
    public static void onWantedChanged(WantedStatusChangedEvent event) {
        ...
        if (!warrant.open()) {
            ...
            CrimeCapabilities.get(offender).ifPresent(PlayerCrimeData::incrementPriorWarrants);
        }
        CrimeDebug.crime("warrant {} for {} rev {}", warrant.open() ? "opened" : "closed", id, warrant.revision());
        MinecraftForge.EVENT_BUS.post(new WarrantChangedEvent(id, warrant,
    ...
    public static void onCrimeCommitted(CrimeCommittedEvent event) {
        ...
        MinecraftForge.EVENT_BUS.post(new WarrantChangedEvent(id, warrant, WarrantChangedEvent.Change.REVISED));
    }
}
